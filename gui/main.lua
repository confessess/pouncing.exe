...
function GUIMain.Create(screenGui, moduleManager)
	...
	if not GUI then
		warn("[Pouncing] GUI Framework not found in module manager!")
		return nil
	end
	...
	if not ok then warn("[Pouncing] Aimbot tab error: " .. tostring(err)) end
	...
	local ok2, err2 = pcall(function()
		...
		local function SetupColorPicker(btn, colorKey)
			...
			btn.MouseButton1Click:Connect(function()
				if ColorPicker:IsOpen() then ColorPicker:Close(); task.wait(0.05) end
	...
	if not ok2 then warn("[Pouncing] ESP tab error: " .. tostring(err2)) end
	...
	if not ok3 then warn("[Pouncing] Gun tab error: " .. tostring(err3)) end
	...
	if not ok4 then warn("[Pouncing] Misc tab error: " .. tostring(err4)) end
	...
	if not ok5 then warn("[Pouncing] Config tab error: " .. tostring(err5)) end
